...
This agent implements intelligent interruption handling to filter out user backchanneling (e.g., "yeah", "ok", "hmm") while the agent is speaking.

- **Soft Interruptions**: If the user says a word from the ignore list while the agent is speaking, the agent will **continue speaking** and ignore the input.
- **Active Interruptions**: If the user says something else (e.g., "stop", "wait", or a full sentence), the agent will stop speaking and listen.
- **Silence Handling**: If the agent is silent, "yeah" or "ok" are treated as valid inputs.

**Configuration**:
The ignore list is currently defined in `AgentActivity._should_ignore_transcript`.
Default values: `yeah`, `ok`, `hmm`, `right`, `uh-huh`, `yes`.
